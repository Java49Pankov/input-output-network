...
class ConsoleInputOutputTest {
	...
	@Test
	void testReadObject() {
		int[] numbers = io.readObject("Enter two integer numbers separated by #", "Must be two numbers", str -> {
			String[] numbersStr = str.split("#");
			return new int[] { Integer.parseInt(numbersStr[0]), Integer.parseInt(numbersStr[1]) };
		});
	...
	void testReadLongRange() {
		long number = io.readLong("Enter long number", "Wrong number", Integer.MAX_VALUE, Long.MAX_VALUE);
		io.writeLine("Entered long number is " + number);
	}

	@Test
	void testReadStringPredicate() {
		String line = io.readString("Enter string with 4 symbols", "Must be 4 symbols", str -> str.length() == 4);
		io.writeLine("Entered string is " + line);
	}

	@Test
	void testReadStringOptions() {
		String line = io.readString("Enter either apple or orange", "Neither apple nor orange",
	...
	void testReadDateRange() {
		LocalDate date = io.readDate("Enter any date in the past", "Wrong past date", LocalDate.MIN,
